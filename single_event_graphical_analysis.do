...
twoway (connect car event_time), xline(0) title("Figure 3. Average Cumulative Abnormal Returns: Bin Laden Death", size(1/100)) xtitle("Days from Event Time") ytitle("CAAR from day -19 to 20")
...
** BY AIRLINES **
twoway (connect car event_time if security==1) (connect car event_time if security==2) (connect car event_time if security==3) (connect car event_time if security==4) (connect car event_time if security==5) (connect car event_time if security==6) (connect car event_time if security==7) (connect car event_time if security==8) (connect car event_time if security==9), legend(label(1 "American Airlines") label(2 "Allegiant") label(3 "Alaska Airlines") label(4 "Delta Airlines") label(5 "Hawaiian Airlines") label(6 "Jetblue Airways") label(7 "Southwest Airlines") label(8 "SkyWest") label(9 "United Airlines")) xline(0) title("Figure 4. Cumulative Abnormal Returns by Airlines: Bin Laden Death", size(1/100)) xtitle("Days from Event Time") ytitle("CAR from day -19 to 20")


twoway (connect car event_time, by(security))
