...
use "airlines_sp.dta", clear
append using "airlines_sp_2010.dta"
...
sum day_group if day == 2 & month ==5 & year == 2011
...
gen event_window = 0 
replace event_window = 1 if event_time>=-19 & event_time<20
*replace event_window = 1 if event_time>=-10 & event_time<11

*generate estimation window

gen estimation_window = 0
replace estimation_window = 1 if event_time<-19 & event_time >=-119
...
save "airlines_ff3fm_coefficients.dta", replace
...
gen event_window = 0 
replace event_window = 1 if event_time>=0 & event_time<21
*replace event_window = 1 if event_time>=-10 & event_time<11

*generate estimation window

gen estimation_window = 0
replace estimation_window = 1 if event_time<0 & event_time >=-100
*replace estimation_window = 1 if event_time<-10 & event_time >=-110
...
save "airlines_ff3fm_coefficients_old.dta", replace
